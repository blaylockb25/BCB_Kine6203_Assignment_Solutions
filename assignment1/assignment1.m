...
%
% Answer: (clear all essentially factory resets the matlab envirnment erasing all previous applications)
%
...
%
% Answer: anova1. This function performs a one-way anova test to determine the significance in the difference between two or more groups thus proving/disproving the null hypothosis.
...
%
% Answer: the two major types of loops are for and while loops the
% difference between them is that for loops run a specific number of
% iterations and while loops run as long as a certain condition remains
% true.
% 
...
%
% Answer: Nan stands for not a number you could get NaN as an answer when
% there are missing data points in a matrix
%
...
originalWeightLb = originalWeightkg*2.2;
who;
whos
...
decimalPlace = 2.5;
nonDecimalPlace = cast(decimalPlace, 'int32');
whos;
...
aVec = abs(originalVec);

% - find the maximum.
mVec = max(originalVec);
...
base1 = 2;
base2 = 3;
height = 4; % Assign a value to height
trapArea = ((base1 + base2) / 2) * height; % Calculate the area of the trapezoid
...
totCosts = input("total cost");
ROI = (totRev - totCosts) / totCosts * 100;
